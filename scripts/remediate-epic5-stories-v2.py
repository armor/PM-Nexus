...
This version parses the DOCUMENTATION.md file to get the original AC items.
"""
...
DOCUMENTATION_FILE = Path("/Users/phillipboles/Development/armor-argus/.bmad/epics/EPIC-5/DOCUMENTATION.md")

# Parse DOCUMENTATION.md to extract story AC items
def parse_documentation():
    """Parse the DOCUMENTATION.md file to extract AC items for each story."""
    with open(DOCUMENTATION_FILE, 'r') as f:
        content = f.read()

    stories = {}

    # Split by story headers (#### UX-XXX: Title)
    # First find all header positions
    header_pattern = r'^#### UX-(\d+): (.+)$'
    headers = list(re.finditer(header_pattern, content, re.MULTILINE))

    for i, header_match in enumerate(headers):
        story_num = header_match.group(1).zfill(3)
        story_title = header_match.group(2).strip()

        # Get content from this header to the next (or end)
        ...
        # Extract functional requirements (format: - [ ] FUNC-X: Description)
        func_pattern = r'- \[ \] (FUNC-\d+): ([^\n]+)'
        func_matches = re.findall(func_pattern, story_content)

        stories[story_num] = {
            'title': story_title,
            'func_reqs': [{'id': m[0], 'text': m[1].strip()} for m in func_matches]
        }
...
def parse_story(content: str) -> Dict:
    ...
    if coord_section:
        for line in coord_section.group(1).strip().split('\n'):
            ...
            if len(parts) >= 2:
                key = parts[0]
                val = parts[1].replace('`', '')  # Remove backticks
                data["coordination"][key] = val
...
def generate_remediated_story(story_dir: Path, parsed_data: Dict, doc_stories: Dict) -> str:
    ...
    context = STORY_CONTEXT.get(story_key, {
        "action": parsed_data.get("story_text", "implement the required functionality").lower(),
        "benefit": "the feature works as expected",
        "value": f"Delivers required functionality",
        "metric": "Feature works as specified",
        "research": "Users require this functionality"
    })

    # Get AC items from DOCUMENTATION.md
    story_num = story_name.split('-')[0].zfill(3)
    doc_story = doc_stories.get(story_num, {})
    func_reqs = doc_story.get('func_reqs', [])
    ...
    content.append("|-------|-------|")
    worktree = parsed_data['coordination'].get('Worktree', f'worktrees/argus-ui-{story_name}')
    branch = parsed_data['coordination'].get('Branch', story_name)
    content.append(f"| Worktree | `{worktree}` |")
    content.append(f"| Branch | `{branch}` |")
    ...
    if func_reqs:
        for i, ac in enumerate(func_reqs, 1):
            gwt = generate_ac_given_when_then(ac["text"], parsed_data.get("story_text", ""))
        ...
        # Fallback if no AC found in documentation
...
def remediate_story(story_dir: Path, doc_stories: Dict) -> bool:
    ...
    try:
        ...
        new_content = generate_remediated_story(story_dir, parsed, doc_stories)
...
def main():
    """Main entry point."""
    print("=" * 60)
    print("EPIC-5-UX Story Remediation (v2 - with DOCUMENTATION.md)")
    print("=" * 60)
    print()

    # Parse DOCUMENTATION.md for original AC items
    print("Parsing DOCUMENTATION.md for original AC items...")
    doc_stories = parse_documentation()
    print(f"Found {len(doc_stories)} stories in documentation")
    print()
    ...
    for i, story_dir in enumerate(story_dirs, 1):
        ...
        if remediate_story(story_dir, doc_stories):
            success += 1
            print(f"  OK")
        else:
            failed += 1
